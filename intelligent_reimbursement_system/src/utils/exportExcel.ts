...
function buildSheet(_category: string, records: ReimbursementRecord[]) {
  const detailLabels: string[] = [];
  for (const r of records) {
    if (r.detail && r.detail.length > 0) {
      r.detail.forEach((d) => {
        if (!detailLabels.includes(d.label)) detailLabels.push(d.label);
      });
      break;
    }
  }
  ...
  // 共计行（红色）
  const subtotalRow: ReturnType<typeof makeCell>[] = Array(colCount).fill(
    makeCell("", RED_CELL),
  );
